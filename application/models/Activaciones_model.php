...
class Activaciones_model extends CI_Model 
{
	...
	public function GetInformacionCUPsGas($CodCup) 
    {
        $sql = $this->db->query("SELECT a.CodProComCup,a.CodProComCli,b.CodProCom,c.TipProCom,d.CodConCom, e.RazSocCli,e.NumCifCli,f.CupsGas AS CUPsName,f.CodCupGas AS CodCups,g.NomTarGas AS NomTar,h.DesPro,date_format(a.FecActCUPs,'%d/%m/%Y') as FecActCUPs,date_format(a.FecVenCUPs,'%d/%m/%Y') as FecVenCUPs,a.ConCup,d.FecVenCon,a.TipCups FROM T_Propuesta_Comercial_CUPs a 
			...
			JOIN T_Contrato d ON d.CodProCom=c.CodProCom
			join T_Cliente e ON e.CodCli=b.CodCli 
			JOIN T_CUPsGas f ON f.CodCupGas=a.CodCup 
			JOIN T_TarifaGas g ON g.CodTarGas=a.CodTar
			JOIN T_Producto h ON h.CodPro=c.CodPro where a.CodCup='$CodCup' AND a.TipCups=1 AND c.TipProCom=1 OR a.CodCup='$CodCup' AND a.TipCups=2 AND c.TipProCom=2 
			...
			JOIN T_CUPsGas f ON f.CodCupGas=a.CodCup 
			JOIN T_TarifaGas g ON g.CodTarGas=a.CodTar
			JOIN T_Producto h ON h.CodPro=c.CodPro where a.CodCup='$CodCup' AND a.TipCups=2 AND c.TipProCom=3 
