app.controller('Controlador_Bancos', ['$http', '$scope', '$filter','$route','$interval', '$controller','$cookies','ServiceProvincias', Controlador]);
function Controlador($http,$scope,$filter,$route,$interval,controller,$cookies,ServiceProvincias)
{
	...
	scope.TBancos=undefined;	
	scope.TBancosBack=undefined;
	...
	{		
		$("#crear_banco").removeClass( "loader loader-default" ).addClass( "loader loader-default is-active");
		var url=base_urlHome()+"api/Configuraciones_Generales/crear_banco/";
		$http.post(url,scope.fdatos).then(function(result)
		{
			scope.nID=result.data.CodBan;
			if(scope.nID>0)
			{
				console.log(result.data);				
				$("#crear_banco").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
				bootbox.alert({
	...
	scope.cargar_lista_bancos=function()
	{
		$("#cargando").removeClass( "loader loader-default" ).addClass( "loader loader-default  is-active");
		var url = base_urlHome()+"api/Configuraciones_Generales/list_bancos/";
		$http.get(url).then(function(result)
		{
			if(result.data!=false)
			{
				...
				scope.TBancos=result.data;
				scope.TBancosBack=result.data;					
				$scope.totalItems = scope.TBancos.length; 
				$scope.numPerPage = 50;  
				...
				{  
					...
					index = scope.TBancos.indexOf(value);  
					return (begin <= index && index < end);  
				};				
				console.log(scope.TBancos);
			}
			else
			{
				$("#cargando").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
	...
	scope.buscarXID=function()
	{
		...
		var url = base_urlHome()+"api/Configuraciones_Generales/buscar_xID_Bancos/CodBan/"+scope.nID;
		$http.get(url).then(function(result)
		{
			if(result.data!=false)
			{
	...
	scope.borrar_row=function(index,id)
	{
		...
		callback: function (result) 
		{
			...
			{					
				...
				var url = base_urlHome()+"api/Configuraciones_Generales/borrar_row_bancos/CodBan/"+id;
	...
	scope.borrar=function()
	{
		...
		callback: function (result) 
		{
			...
			{					
				$("#borrando").removeClass( "loader loader-default" ).addClass( "loader loader-default  is-active");
				var url = base_urlHome()+"api/Configuraciones_Generales/borrar_row_bancos/CodBan/"+scope.fdatos.CodBan;
				$http.delete(url).then(function(result)
				{
					if(result.data!=false)
					{
	...
	scope.filtrar = function(expresion)
	{
		if (expresion.length>0)
		{
			...
			scope.TBancos = $filter('filter')(scope.TBancosBack, {CodEur: expresion});				
			$scope.totalItems = scope.TBancos.length; 
			$scope.numPerPage = 50;  
			$scope.paginate = function (value) 
			{  
		...
		{
			//scope.cargar_lista_bancos();
			scope.TBancos=scope.TBancosBack;
		}
	}	
