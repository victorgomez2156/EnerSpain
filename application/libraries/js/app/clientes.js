...
scope.cargar_lista_clientes=function()
{
	...
	$http.get(url).then(function(result)
	{
		...
		{			
			Swal.fire({title:"Error",text:"No hemos encontrado clientes registrados.",type:"error",confirmButtonColor:"#188ae2"});
			scope.Tclientes=[];
			scope.TclientesBack=[];
		}
	},function(error)
...
{      
	...
	else
	{
		...
		{			
			...
			if(result.data!=false)
			{
				Swal.fire({title:"Encontrado.",text:"El Cliente ya se encuentra registrado.",type:"info",confirmButtonColor:"#188ae2"});					
			}
			else
			{
...
$scope.SubmitFormFiltrosClientes = function(event) 
{
	if(scope.tmodal_data.tipo_filtro==1)
	{
		...
		{
			Swal.fire({title:"Error.",text:"Debe de seleccionar un tipo de cliente.",type:"error",confirmButtonColor:"#188ae2"});
			return false;
		}
		$scope.predicate = 'id';  
		$scope.reverse = true;						
	...
	if(scope.tmodal_data.tipo_filtro==2)
	{
		...
		{
			Swal.fire({title:"Error.",text:"Debe de seleccionar un Sector.",type:"error",confirmButtonColor:"#188ae2"});
			return false;
		}
		$scope.predicate = 'id';  
		$scope.reverse = true;						
	...
	if(scope.tmodal_data.tipo_filtro==3)
	{
		...
		{
			Swal.fire({title:"Error.",text:"Debe de seleccionar una Provincia.",type:"error",confirmButtonColor:"#188ae2"});
			return false;
		}
		$scope.predicate = 'id';  
		$scope.reverse = true;						
	...
	if(scope.tmodal_data.tipo_filtro==4)
	{
		...
		{
			Swal.fire({title:"Error.",text:"Debe de seleccionar una Localidad.",type:"error",confirmButtonColor:"#188ae2"});
			return false;
		}
		$scope.predicate = 'id';  
		$scope.reverse = true;						
	...
	if(scope.tmodal_data.tipo_filtro==5)
	{
		...
		{
			Swal.fire({title:"Error.",text:"Debe de seleccionar un Comercial.",type:"error",confirmButtonColor:"#188ae2"});
			return false;
		}
		$scope.predicate = 'id';  
		$scope.reverse = true;						
	...
	if(scope.tmodal_data.tipo_filtro==6)
	{
		...
		{
			Swal.fire({title:"Error.",text:"Debe de seleccionar un Colaborador.",type:"error",confirmButtonColor:"#188ae2"});
			return false;
		}
		$scope.predicate = 'id';  
		$scope.reverse = true;						
	...
	if(scope.tmodal_data.tipo_filtro==7)
	{
		...
		{
			Swal.fire({title:"Error.",text:"Debe de seleccionar un Estatus.",type:"error",confirmButtonColor:"#188ae2"});
			return false;
		}
		$scope.predicate = 'id';  
		$scope.reverse = true;						
...
scope.validar_opcion=function(index,opcion,datos)
{
	...
	if(opcion==1)
	{
		...
		/*scope.opciones_clientes[index]=undefined;
		Swal.fire({title:"Datos Basicos",text:"Ve a la Pestaña Datos Basicos Para Ver Los Datos del Cliente.",type:"info",confirmButtonColor:"#188ae2"});
		scope.fdatos=datos;
		scope.fdatos.CodProSoc=datos.CodProSoc2;
		scope.filtrarLocalidad();
		scope.fdatos.CodLocSoc=datos.CodLocSoc1;
		scope.validate_cif=undefined;
		scope.validate_info=1;
		scope.fdatos.CodTipCli=datos.CodTipCliSoc2;
		scope.fdatos.CodSecCli=datos.CodSecCliSoc2;
		scope.fdatos.CodTipViaSoc=datos.CodTipViaSoc1;
		scope.fdatos.ZonPosSoc=datos.CPLocSoc;
		scope.fdatos.CodCom=datos.CodCom2;
		scope.fdatos.CodProFis=datos.CodProFis2;
		scope.filtrarLocalidadFisc();
		scope.fdatos.CodLocFis=datos.CodLocFis2;
		scope.fdatos.CodCol=datos.CodCol2;
		scope.fdatos.CodTipViaFis=datos.CodTipViaFis2;
		if(scope.fdatos.CodProSoc!=scope.fdatos.CodLocFis)
		{
	...
	if(opcion==2)
	{
        ...
        /*scope.opciones_clientes[index]=undefined;
        Swal.fire({title:"Datos Basicos",text:"Ve a la Pestaña Datos Basicos Para Editar Los Datos del Cliente.",type:"info",confirmButtonColor:"#188ae2"});
        scope.fdatos=datos;
        scope.validate_cif=undefined;
        scope.validate_info=undefined;
        if(scope.fdatos.RazSocCli!=scope.fdatos.NomComCli)
		{
		...
		scope.fdatos.CodLocSoc=datos.CodLocSoc1;
		scope.fdatos.ZonPosSoc=datos.CPLocSoc;	
		if(scope.fdatos.CodProSoc2!=scope.fdatos.CodProFis2)
		{
			scope.fdatos.distinto_a_social=true;
		}
		else
		...
		scope.filtrarLocalidadFisc();
		scope.fdatos.CodLocFis=datos.CodLocFis2;
		scope.fdatos.ZonPosFis=datos.CPLocFis;
		scope.fdatos.CodCom=datos.CodCom2;
		scope.fdatos.CodCol=datos.CodCol2;*/
	...
	if(opcion==4)
	{
		...
		scope.tmodal_data.FechBlo=scope.fecha_server;
...
{ 
	...
	confirmButtonText:"Bloquear"}).then(function(t)
	{
        if(t.value==true)
        {
			...
			var FechBlo= (scope.datos_update.FechBlo).split("/");
			if(FechBlo.length<3)
			{
			...
			{		
				...
				valuesStart=scope.datos_update.FechBlo.split("/");
