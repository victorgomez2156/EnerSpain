...
function Controlador($http, $scope, $filter, $route, $interval, $controller, $cookies) {
   ...
   scope.setValue = function(index,$event,result,metodo)
   {
    ...
    {   
        ...
        {var title ='¿Estás seguro de generar esta gestión comercial?';
         var loader="Guardando";}
        if(scope.fdatos.tipo=="ver"||scope.fdatos.tipo=="editar")
        {var title ='¿Estás seguro de cambiar esta gestión comercial?';
    ...
    scope.validar_campos_gestion = function() 
     {
            ...
            if (FecGesGen.length < 3) {
                ...
                if (FecGesGen[1].length > 2 || FecGesGen[1].length < 2) {
                 ...
                 if (dateStart > dateEnd) {
                     Swal.fire({ title: "La Fecha de registro no puede ser mayor al " + scope.FechaServer + " Por Favor Verifique he intente nuevamente.", type: "error", confirmButtonColor: "#188ae2" });
                     return false;
                 }
        ...
        {
            Swal.fire({ title: "Debe elegir un tipo de suministro.", type: "error", confirmButtonColor: "#188ae2" });
            return false;
        }
        if(!scope.fdatos.CodCups>0)
        {
            Swal.fire({ title: "Debe elegir un CUPs de la lista.", type: "error", confirmButtonColor: "#188ae2" });
            return false;
        }
        if(scope.fdatos.MecGesGen==undefined)
        {
            Swal.fire({ title: "Debe elegir un tipo de mecanismo.", type: "error", confirmButtonColor: "#188ae2" });
            return false;
        }
        if(scope.fdatos.PreGesGen==undefined || scope.fdatos.PreGesGen==null|| scope.fdatos.PreGesGen=='')            
        {
            Swal.fire({ title: "El precio es requerido.", type: "error", confirmButtonColor: "#188ae2" });
            return false;
        }
