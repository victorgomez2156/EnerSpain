...
if($route.current.$$route.originalPath=="/Add_Puntos_Suministros/:CodCli")
{
    ...
    {
        var searchText_len = scope.fpuntosuministro.CPLocSoc.trim().length;
        if (searchText_len >=3) 
        {
            if(metodo==1)
            {
               var url= base_urlHome()+"api/Clientes/LocalidadCodigoPostal/CPLoc/"+scope.fpuntosuministro.CPLocSoc;
            }
            $http.get(url).then(function(result) 
            {
                if (result.data != false) 
    ...
    scope.setValueCPLoc = function(index, $event, result, metodo) 
    {
        if (metodo == 1) 
        {
            ...
            console.log(result);
            console.log(metodo);
            scope.fpuntosuministro.CodProPunSum=scope.searchResultCPLoc[index].CodPro;
            scope.BuscarLocalidadesPunSun(scope.fpuntosuministro.CodProPunSum,2);
            scope.fpuntosuministro.CodLocPunSum=scope.searchResultCPLoc[index].CodLoc;
            scope.fpuntosuministro.CPLocSoc= scope.searchResultCPLoc[index].CPLoc;
            scope.searchResultCPLoc = {};
            $event.stopPropagation();
        }
    }
