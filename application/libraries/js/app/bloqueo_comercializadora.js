app.controller('Controlador_MotBloCom', ['$http', '$scope', '$filter','$route','$interval', '$controller','$cookies', Controlador]);
function Controlador($http,$scope,$filter,$route,$interval,$controller,$cookies)
{
	...
	scope.TMotBloCom=undefined;
	scope.TMotBloComBack=undefined;
	scope.CodMotBloCom=true;
	...
	{		
		...
		var url=base_urlHome()+"api/Configuraciones_Generales/crear_motivo_BloCom/";
		$http.post(url,scope.fdatos).then(function(result)
		{	
			scope.nID=result.data.CodMotBloCom;
			if(result.data!=false)
			{
				console.log(result.data);				
				$("#crear_motivo_PumSum").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
				Swal.fire({title:"Exito.",text:"El Motivo de Bloqueo de comercializadora a sido registrado correctamente.",type:"success",confirmButtonColor:"#188ae2"});				
				scope.buscarXID();				
			}
			else
			{
	...
	scope.cargar_lista_motivos_Comercializadora=function()
	{
		$("#cargando").removeClass( "loader loader-default" ).addClass( "loader loader-default  is-active");
		var url = base_urlHome()+"api/Configuraciones_Generales/list_motivos_Comercializadora/";
		$http.get(url).then(function(result)
		{
			if(result.data!=false)
			{
				...
				$scope.totalItems = scope.TMotBloCom.length; 
				$scope.numPerPage = 50;  
				...
				{  
					...
					index = scope.TMotBloCom.indexOf(value);  
					return (begin <= index && index < end);  
				};				
				console.log(scope.TMotBloCom);
			}
			else
			{
				$("#cargando").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
				Swal.fire({title:"Error",text:"No hemos encontrado motivos de bloqueos de comercializadoras.",type:"error",confirmButtonColor:"#188ae2"});
			}
		},function(error)
		{
			if(error.status==404 && error.statusText=="Not Found")
	...
	scope.buscarXID=function()
	{
		...
		var url = base_urlHome()+"api/Configuraciones_Generales/buscar_xID_MotBloCom/CodMotBloCom/"+scope.nID;
		$http.get(url).then(function(result)
		{
			if(result.data!=false)
			{
	...
	scope.borrar_row=function(index,id)
	{
		...
		confirmButtonText:"OK"}).then(function(t)
		{
	        if(t.value==true)
	    	{
				...
				var url = base_urlHome()+"api/Configuraciones_Generales/borrar_row_MotBloCom/CodMotBloCom/"+id;
				$http.delete(url).then(function(result)
				{
					if(result.data!=false)
					{
						...
						scope.TMotBloCom.splice(index,1);
					}
					else
					{
						$("#borrando").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
	...
	scope.borrar=function()
	{
		...
		confirmButtonText:"OK"}).then(function(t)
		{
		    ...
		    {
		      	$("#borrando").removeClass( "loader loader-default" ).addClass( "loader loader-default  is-active");
				var url = base_urlHome()+"api/Configuraciones_Generales/borrar_row_MotBloCom/CodMotBloCom/"+scope.fdatos.CodMotBloCom;
				$http.delete(url).then(function(result)
				{
					if(result.data!=false)
					{
