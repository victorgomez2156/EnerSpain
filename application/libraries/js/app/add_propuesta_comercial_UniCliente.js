 app.controller('Controlador_Add_PropuestasUniCliente', ['$http', '$scope', '$filter', '$route', '$interval', '$controller', '$cookies', '$compile', Controlador]);
 ...
 function Controlador($http, $scope, $filter, $route, $interval, $controller, $cookies, $compile) {
     ...
     scope.validar_campos_UniCliente = function() 
    {
     ...
     scope.AgregarCUPs=function(MetodoCUPs)
     {
        ...
        {  
            ...
            var url=base_urlHome()+"api/PropuestaComercial/GetCUPsTar/MetodoCUPs/"+MetodoCUPs+"/CodCli/"+scope.fdatos.CodCli;
    ...
    scope.validar_campos_CUPsElectricos = function() 
    {
        ...
        if (!scope.fdatos.CodCupSEle > 0) {
             scope.toast('error','Debe Seleccionar un CUPs Eléctrico.','');
             return false;
        }
    ...
    if($route.current.$$route.originalPath=="/Add_Propuesta_Comercial_UniCliente_MultiPunto/:CodCli/:Tipo")
    {
    ...
    scope.realizar_filtro = function(metodo, PrimaryKey) {
     ...
     scope.BuscarXIDProComUniCliente=function()
     {
        var url = base_urlHome()+"api/PropuestaComercial/GetProComUniCli/CodProCom/"+scope.CodProCom;
        $http.get(url).then(function(result)
        {
            if(result.data!=false)
            {
     ...
     if($route.current.$$route.originalPath=="/Ver_Propuesta_Comercial_UniCliente/:CodProCom/:Tipo" || $route.current.$$route.originalPath=="/Edit_Propuesta_Comercial_UniCliente/:CodProCom/:Tipo")
     {
        scope.BuscarXIDProComUniCliente();
     }
