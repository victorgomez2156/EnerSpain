app.controller('Controlador_Tipo_Clientes', ['$http', '$scope', '$filter','$route','$interval', '$controller','$cookies', Controlador]);
function Controlador($http,$scope,$filter,$route,$interval,$controller,$cookies)
{
	...
	{		
		$("#crear_tipo_cliente").removeClass( "loader loader-default" ).addClass( "loader loader-default is-active");
		var url=base_urlHome()+"api/Configuraciones_Generales/crear_tipo_cliente/";
		$http.post(url,scope.fdatos).then(function(result)
		{
			scope.nID=result.data.CodTipCli;
			if(scope.nID>0)
			{
				console.log(result.data);				
				$("#crear_tipo_cliente").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
				bootbox.alert({
				message: "El Tipo Cliente a sido registrada correctamente.",
				size: 'middle'});
				scope.buscarXID();				
			}
			else
	...
	scope.cargar_lista_tipo_clientes=function()
	{
		...
		var url = base_urlHome()+"api/Configuraciones_Generales/list_tipo_clientes/";
		$http.get(url).then(function(result)
		{
			...
			else
			{
				$("#cargando").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
				bootbox.alert({
				message: "No hemos encontrado provincias registradas.",
				size: 'middle'});
				scope.Tipo_Cliente=undefined;
			}
		},function(error)
	...
	scope.buscarXID=function()
	{
		...
		var url = base_urlHome()+"api/Configuraciones_Generales/buscar_xID_Tipo_Cliente/CodTipCli/"+scope.nID;
		$http.get(url).then(function(result)
		{
			if(result.data!=false)
			{
	...
	scope.borrar_row=function(index,id)
	{
		...
		callback: function (result) 
		{
			...
			{					
				...
				var url = base_urlHome()+"api/Configuraciones_Generales/borrar_row_tipo_cliente/CodTipCli/"+id;
				$http.delete(url).then(function(result)
				{
					if(result.data!=false)
					{
						...
						size: 'middle'});
						scope.Tipo_Cliente.splice(index,1);
					}
					else
					{
						$("#borrando").removeClass( "loader loader-default is-active" ).addClass( "loader loader-default" );
	...
	scope.borrar=function()
	{
		...
		callback: function (result) 
		{
			...
			{					
				$("#borrando").removeClass( "loader loader-default" ).addClass( "loader loader-default  is-active");
				var url = base_urlHome()+"api/Configuraciones_Generales/borrar_row_tipo_cliente/CodTipCli/"+scope.fdatos.CodTipCli;
				$http.delete(url).then(function(result)
				{
					if(result.data!=false)
					{
