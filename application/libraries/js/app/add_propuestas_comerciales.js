 ...
 function Controlador($http, $scope, $filter, $route, $interval, $controller, $cookies, $compile) {
     ...
     scope.realizar_filtro = function(metodo, PrimaryKey) {
         ...
         $http.get(url).then(function(result) {
             if (result.data != false) {
                 ...
                 if (metodo == 1) {
                     Swal.fire({ title: "Error", text: "No se encontraron productos asignado a esta Comercializadora.", type: "error", confirmButtonColor: "#188ae2" });
                 ...
                 if (metodo == 2) {
                     Swal.fire({ title: "Error", text: "No se encontraron anexos asignados a este producto.", type: "error", confirmButtonColor: "#188ae2" });
                     scope.List_Anexos = [];
                     scope.fdatos.CodAnePro = undefined;
                     scope.fdatos.TipPre = undefined;
                 }
     ...
     $scope.submitFormPropuesta = function(event) {
         ...
         if (scope.fdatos.tipo == "nueva" || scope.fdatos.tipo == "renovar") {
             var title = '¿Seguro que desea registrar la Propuesta Comercial?';
             var loader = "Guardando";
         }
         if (scope.fdatos.tipo == "ver" || scope.fdatos.tipo == "editar") {
             var title = '¿Seguro que desea actualizar la información de la Propuesta Comercial?';
     ...
     scope.validar_campos_propuestas = function() {
         ...
         if (scope.FecProCom == null || scope.FecProCom == undefined || scope.FecProCom == '') {
             ...
             if (FecProCom.length < 3) {
                 Swal.fire({ title: "El formato Fecha de la Propuesta debe ser DD/MM/YYYY", type: "error", confirmButtonColor: "#188ae2" });
