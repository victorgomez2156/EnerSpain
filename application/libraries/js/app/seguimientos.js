   ...
   scope.fetchClientes = function()
    {
        ...
        if(searchText_len > 0)
        {
            ...
            $http.post(url,scope.fdatos).then(function(result)
            {
                ...
                {scope.searchResult = result.data;console.log(scope.searchResult);}
                else{Swal.fire({ title: "Error", text: "No se encontraron clientes registrados.", type: "error", confirmButtonColor: "#188ae2" });                    
                    scope.searchResult = {};}
            }, function(error)
            {
    ...
    scope.search_seguimientos=function()
    {
      //$("#" + titulo).removeClass("loader loader-default").addClass("loader loader-default is-active");
        ...
        $http.get(url).then(function(result)
        {
            ...
            {
               Swal.fire({ title: "Error", text: "No se encontraron seguimientos asignados", type: "error", confirmButtonColor: "#188ae2" }); 
               scope.T_Seguimientos=[];
               scope.T_SeguimientosBack=[];
            }
            //$("#" + titulo).removeClass("loader loader-default is-active").addClass("loader loader-default");
