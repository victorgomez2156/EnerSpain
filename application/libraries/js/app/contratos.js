    ...
    scope.validar_campos_contratos = function() 
    {
         ...
         if (scope.FecIniCon == null || scope.FecIniCon == undefined || scope.FecIniCon == ''){
            ...
            if (FecIniCon.length < 3) {
                 Swal.fire({ title: "El Formato de Fecha de Inicio debe Ser EJ: DD/MM/YYYY.", type: "error", confirmButtonColor: "#188ae2" });
                 event.preventDefault();
                 return false;
            }else{
                if (FecIniCon[0].length > 2 || FecIniCon[0].length < 2) {
                    Swal.fire({ title: "Por Favor Corrija el Formato del dia en la Fecha de Inicio deben ser 2 números solamente. EJ: 01", type: "error", confirmButtonColor: "#188ae2" });
                    event.preventDefault();
                    return false;
                }
                if (FecIniCon[1].length > 2 || FecIniCon[1].length < 2) {
                     Swal.fire({ title: "Por Favor Corrija el Formato del mes de la Fecha de Inicio deben ser 2 números solamente. EJ: 01", type: "error", confirmButtonColor: "#188ae2" });
                     event.preventDefault();
                     return false;
                 }
                 if (FecIniCon[2].length < 4 || FecIniCon[2].length > 4) {
                     Swal.fire({ title: "Por Favor Corrija el Formato del ano en la Fecha de Inicio Ya que deben ser 4 números solamente. EJ: 1999", type: "error", confirmButtonColor: "#188ae2" });
                     event.preventDefault();
                     return false;
                 }
         ...
         if (!scope.fdatos.DurCon > 0) {
             Swal.fire({ title: "Debe seleccionar un tiempo de duración del contrato.", type: "error", confirmButtonColor: "#188ae2" });
             return false;
         }
         ...
         if (scope.FecVenCon == null || scope.FecVenCon == undefined || scope.FecVenCon == ''){
            ...
            if (FecVenCon.length < 3) {
                 Swal.fire({ title: "El Formato de Fecha de Vencimiento debe Ser EJ: DD/MM/YYYY.", type: "error", confirmButtonColor: "#188ae2" });
                 event.preventDefault();
                 return false;
            }else{
                if (FecVenCon[0].length > 2 || FecVenCon[0].length < 2) {
                    Swal.fire({ title: "Por Favor Corrija el Formato del dia en la Fecha de Vencimiento deben ser 2 números solamente. EJ: 01", type: "error", confirmButtonColor: "#188ae2" });
                    event.preventDefault();
                    return false;
                }
                if (FecVenCon[1].length > 2 || FecVenCon[1].length < 2) {
                     Swal.fire({ title: "Por Favor Corrija el Formato del mes de la Fecha de Vencimiento deben ser 2 números solamente. EJ: 01", type: "error", confirmButtonColor: "#188ae2" });
                     event.preventDefault();
                     return false;
                 }
                 if (FecVenCon[2].length < 4 || FecVenCon[2].length > 4) {
                     Swal.fire({ title: "Por Favor Corrija el Formato del ano en la Fecha de Vencimiento Ya que deben ser 4 números solamente. EJ: 1999", type: "error", confirmButtonColor: "#188ae2" });
                     event.preventDefault();
                     return false;
                 }
     ...
     scope.validar_opcion_contratos=function(index,opcion_select,dato)
     {
        ...
        {           
            if(dato.EstBajCon==3)
            {
                Swal.fire({ title: "Error", text: "Este contrato ya fue renovado.", type: "error", confirmButtonColor: "#188ae2" });
                return false;
            }
            scope.tmodal_data={};
