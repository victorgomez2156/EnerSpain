...
function Controlador($http, $scope, $filter, $route, $interval, $controller, $cookies) {
    ...
    scope.limpiar = function() {
        if (scope.fdatos.CodCom == undefined) {
            ...
            console.log('No hay datos por limpiar');
        }
    }
            ...
            Swal.fire({
                ...
                if (t.value == true) {
                    ...
                    $http.post(url, scope.datos_update).then(function(result) {
                        if (result.data != false) {
                            Swal.fire({ title: "Exito!.", text: "El Comercial ha sido activado de forma correcta", type: "success", confirmButtonColor: "#188ae2" });
            ...
            if (t.value == true) {
                ...
                $http.post(url, scope.datos_update).then(function(result) {
                    ...
                    if (result.data.resultado != false) {
                        console.log(result.data);
                        scope.datos_update = {};
                        Swal.fire({ title: "Exito", text: "Comercial Bloqueado de forma correcta", type: "success", confirmButtonColor: "#188ae2" });
