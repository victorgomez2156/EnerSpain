...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    protected function _detect_api_key()
    {
		...
		//var_dump($this->_args[$api_key_variable]);
		//(($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
			$row = $this->rest->db->where(config_item('rest_key_column'),$key)->get(config_item('rest_keys_table'))->row();
			//var_dump(empty($row));
            if (empty($row)){
				return false;
			...
			//modificada porque retornada un valor $row cuando en realidad deberia retornar un valor TRUE
    ...
    protected function _check_access()
    {
        ...
        $this->rest->db->where('controller', $controller[1]);
